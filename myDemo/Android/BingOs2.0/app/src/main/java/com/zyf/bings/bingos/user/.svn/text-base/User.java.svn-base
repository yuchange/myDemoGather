package com.zyf.bings.bingos.user;
...
public class User {
    ...
    public void initUser(final String type , final LatLng latLng) {
        ...
        memberId = phone;
        if (type != null) {
            String mac = CommonUtils.getWifiMac();
    ...
    public String getMemberFromSP() {
        if (MainBuildConfigure.BUILD_FLAG.equals(MainBuildConfigure.BUILD_HAIER)) {
            if (applicationContext == null) {
                return null;
            }
            SharedPreferences sp = applicationContext.getSharedPreferences(HAIER_PHONE_SP, Context.MODE_PRIVATE);
            return sp.getString(HAIER_PHONE_KEY, "");
        } else if (MainBuildConfigure.BUILD_FLAG.equals(MainBuildConfigure.BUILD_BINGOS)) {
            if (applicationContext == null) {
                return null;
            }
            ...
            return "18336092512"; //15869153840 13840933985 18336092508  18361603511
        }
    }
    ...
    public String getFridgeType() {
        if (MainBuildConfigure.BUILD_FLAG.equals(MainBuildConfigure.BUILD_HAIER)) {
            ...
            SharedPreferences sp = applicationContext.getSharedPreferences(HAIER_PHONE_SP, Context.MODE_PRIVATE);
            return sp.getString(HAIER_FRIDGE_TYPE, "");
        } else {
            return "";
        }
    }
    ...
    private static void checkHaierLogin(final Context context, final CheckMemberCallback callback) {
        if (context instanceof Activity) {
            try {
                Class<?> cls = Class.forName("com.haiersmart.sfnation.bizutils.UserUtils");
                Method method = cls.getMethod("loginToDo", Activity.class, Runnable.class);
